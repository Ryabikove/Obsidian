...
При использовании первого типа синтаксиса ключевое слово [[Java/constructions/управляющие конструкции/циклы/break|break]] необходимо для прерывания проверок, т.к. при удовлетворении одного условия, выполнятся все последующие до первого слова `break`.

Во втором случае используются [[Java/constructions/управляющие конструкции/Лямбда-выражения|лямбда-выражения]], внутри них нет необходимости использовать слова `break`, запись короче по сравнению с предыдущей.
