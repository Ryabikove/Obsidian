...
- [[обучение/JPA & Hibernate/Entity|Entity]] (Сущность) - обычный java класс, помеченный специальными аннотациями, который JPA воспринимает как схему таблицы.
- [[обучение/JPA & Hibernate/EntityManager|EntityManager]] - основной интерфейс, который управляет жизненным циклом объектов (сохраняет, обновляет, ищет и удаляет их)
- [[обучение/JPA & Hibernate/Persistence Provider|Persistence Provider]] - реализация стандарта. JPA - это лишь референс или правила, а реализация правил происходит в библиотеке (например Hibernate)